...
app.secret_key = os.environ.get("XSS_SECRET_KEY", "xss-toolkit-secret-change-me-in-prod")
app.config.update(
...
def api_register():
    ...
    if auth.is_smtp_configured():
        ...
        code = auth.store_pending_verification(username, email, password)
        sent = auth.send_verification_email(email, code)

        if not sent:
            return jsonify({"error": "Failed to send verification email. Check SMTP config."}), 500
        ...
        # No SMTP — create user directly (fallback)
...
@app.route("/api/auth/smtp-status")
def api_smtp_status():
    """Check if SMTP is configured (for frontend to show/hide verification UI)."""
    return jsonify({"configured": auth.is_smtp_configured()})
