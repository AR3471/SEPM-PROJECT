...
function pollScanStatus() {
    ...
    state.scanPollTimer = setInterval(async () => {
        ...
        if (status.logs) {
            ...
            const currentCount = scanLog.querySelectorAll('span').length / 1; // rough count
            status.logs.forEach((entry, i) => {
                // Simple dedup: check if message already in DOM (last 5 entries)
                const lastTexts = Array.from(scanLog.querySelectorAll('span'))
                    .slice(-10)
                    .map(s => s.textContent);
                if (!lastTexts.includes(entry.msg)) {
...
async function toggleC2() {
    const btn = document.getElementById('c2-start-btn');
...
async function togglePayloadServer() {
    ...
    if (!state.psRunning) {
        ...
        btn.textContent = '🔴 Stop Payload Server';
        btn.className = 'btn-danger';
        info.style.display = '';
        ...
        btn.textContent = '📦 Start Payload Server';
